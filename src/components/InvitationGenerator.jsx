...
function formatFirstName(str) {
	...
	if (isCombined) {
		...
		names.forEach((name) => {
			...
			const romanNumeralPattern = /\bM{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})\b/gi;
			let temp = name;
...
const InvitationGenerator = () => {
	...
	const fontBold = new window.FontFace("Gempire", "url(/fonts/Gempire-Bold.woff)", { weight: "bold" });
	const fontRegular = new window.FontFace("Gempire", "url(/fonts/Gempire-Regular.woff)", { weight: "normal" });
	await fontBold.load();
		...
		img.onload = async () => {
			...
			if (andMatch) {
				...
				if (textWidth > 1800) {
					...
					context.fillStyle = "#EFC124";
					// Center both lines
					const firstName = (awardee?.team?.toUpperCase() ?? formatFirstName(awardee.firstName).toUpperCase());
					const lastName = (awardee?.team?.toUpperCase() ? '' : awardee.lastName.toUpperCase());
		...
		<div style={{ padding: "2rem", textAlign: "center" }}>
			...
			>
				<input
					...
					onChange={e => setAwardee(a => ({ ...a, firstName: e.target.value }))}
					style={{ marginRight: 8 }}
				/>
				<input
					type="text"
					...
					onChange={e => setAwardee(a => ({ ...a, lastName: e.target.value }))}
					style={{ marginRight: 8 }}
				/>
				<input
					type="text"
					...
					value={awardee.team || ""}
					onChange={e => setAwardee(a => ({ ...a, team: e.target.value || null }))}
					style={{ marginRight: 8 }}
				/>
			</form>
			<div style={{ display: "flex", justifyContent: "center" }}>
				<canvas
					...
					width={2480}
					height={2404}
					style={{ border: "2px solid #ccc", background: "#fff", maxWidth: "100%" }}
				/>
			</div>
		</div>
	);
